...
[ammountData( Report ammount)] 1 2019-10-01 00:00:00 UTC(18.73) 
    ...
    CALL_FROM_FILE("-noargs")<<"Execution of a simple test with five records only"<<endl;
...
[ammountData( Report ammount)] 10 beautix(43.82) grattol( 6.27) jas(25.71) jessnail( 1.10) kapous( 4.51) kerasys( 1.87) masura( 3.46) runail(41.94) shary( 6.04) staleks(63.33) 
    ...
    CALL_FROM_FILE("-noargs")<<"Execution of a test with 30 records only"<<endl;
}

TEST_F(_01_Basics, Integrated_41_records)  {   
...
[ammountData( Report ammount)] 4 view(47.97) remove_from_cart(20.13) cart( 0.63) purchase(49.32) 
);
    CALL_FROM_FILE("-noargs")<<"Execution of a simple test with 41 records"<<endl;
    LOAD(_expected,((string)"./tests/validation/"+stestname+"-hit.dataset").c_str()); 
    LOAD(_output,((string)"./data/"+stestname+"-hit.dataset").c_str());
    ASSERT_FALSE_R(strcmp(_expected.c_str(), _output.c_str()))<<"The output file of this test with 41 records is not as expected, please have a look at "<<((string)"./tests/validation/"+stestname+"-hit.dataset")<<endl;
}

TEST_F(_01_Basics, Integrated_162_records)  {   
    DEF_EXECUTION_ENVIRONMENT(ECommerce162);
    USE_MEMORY_LEAKS(__VALGRIND__);
    SET_TIMEOUT_S(15);
    EXPECTED_OUTPUT(
[originalES(Original read from dataset)] 162 3503607102617202807
[filteredES(Events indexed)] 99 4040825974372824428
...
[ammountData( Report ammount)] 31 beauty-free( 1.89) blixz( 1.21) bluesky( 3.97) bpw.style(24.23) cosmoprofi( 7.14) depilflax( 7.14) domix( 4.57) entity( 3.40) estel(43.65) f.o.x( 4.19) freedecor( 9.83) haruyama( 3.97) ingarden( 7.34) irisk(43.25) italwax(68.98) jessnail(44.29) konad( 4.19) kosmekka(15.24) lamixx(46.49) masura(17.17) metzger( 3.73) milv( 3.49) orly( 1.98) pnb( 6.61) roubloff( 6.28) runail(10.16) shary( 7.94) shik(20.64) solomeya( 0.87) staleks(69.86) uno(10.32) 
);
    CALL_FROM_FILE("-noargs")<<"Execution of a simple test with 162 records"<<endl;
    LOAD(_expected,((string)"./tests/validation/"+stestname+"-hit.dataset").c_str()); 
    LOAD(_output,((string)"./data/"+stestname+"-hit.dataset").c_str());
    ASSERT_FALSE_R(strcmp(_expected.c_str(), _output.c_str()))<<"The output file of this test with 162 records is not as expected, please have a look at "<<((string)"./tests/validation/"+stestname+"-hit.dataset")<<endl;
}

TEST_F(_01_Basics, Integrated_926_records)  {   
    DEF_EXECUTION_ENVIRONMENT(ECommerce926);
    USE_MEMORY_LEAKS(__VALGRIND__);
    SET_TIMEOUT_S(15);
    EXPECTED_OUTPUT(
[originalES(Original read from dataset)] 500 18248242769591794958
[filteredES(Events indexed)] 500 12964397401011121073
...
[ammountData( Report ammount)] 57 (2790.43) irisk(129.85) farmstay( 0.63) farmona(80.73) kapous(21.90) runail(63.61) masura(57.88) marathon(964.46) strong(1172.22) pnb( 7.38) domix( 4.24) milv( 3.17) artex(15.08) zinger(48.15) yoko( 7.83) ingarden(174.99) uskusi(19.05) jas(51.42) lovely(34.93) enigma( 6.90) max(579.86) uno(134.48) enas( 3.54) jessnail( 8.73) lianail( 6.33) osmo( 9.71) grattol(25.14) estelare( 3.02) lowence(69.84) kinetics(18.74) art-visage( 9.36) provoc(19.71) consly(23.50) eunyul(35.55) lebelage( 3.75) browxenna(29.68) thuya(252.06) italwax(26.04) depilflax(10.54) estel(27.45) beauty-free(18.80) solomeya( 1.75) oniq(139.68) kosmekka(128.88) haruyama(47.64) i-laq(15.88) bioaqua(54.46) emil(55.56) kaaral(59.22) beautyblender(99.04) cnd(34.28) shik(20.64) bpw.style( 0.79) dermal( 4.12) keen(19.56) sunuv(33.33) beautix(31.42)
);
    CALL_FROM_FILE("-noargs")<<"Execution of a simple test with 926 records"<<endl;
    LOAD(_expected,((string)"./tests/validation/"+stestname+"-hit.dataset").c_str()); 
    LOAD(_output,((string)"./data/"+stestname+"-hit.dataset").c_str());
    ASSERT_FALSE_R(strcmp(_expected.c_str(), _output.c_str()))<<"The output file of this test with 926 records is not as expected, please have a look at "<<((string)"./tests/validation/"+stestname+"-hit.dataset")<<endl;
}

TEST_F(_02_Intermediate, Integrated_EMPTY)  {   
    DEF_EXECUTION_ENVIRONMENT(EMPTY);
    USE_MEMORY_LEAKS(__VALGRIND__);
    SET_TIMEOUT_S(15);
    EXPECTED_OUTPUT(
[originalES(Original read from dataset)] 0 6142509188972423790
[filteredES(Events indexed)] 0 6142509188972423790
[hitsData( Report traffic)] 0 
[ammountData( Report ammount)] 0 
);
    CALL_FROM_FILE("-noargs")<<"Execution of a simple test with an empty set of records"<<endl;
}

TEST_F(_02_Intermediate, Integrated_ErrorLoading)  {   
    DEF_EXECUTION_ENVIRONMENT(ErrorLoading);
    USE_MEMORY_LEAKS(__VALGRIND__);
    SET_TIMEOUT_S(15);
    EXPECTED_OUTPUT(Error opening file ./datas/ECommerce5.dataset);
    CALL_FROM_FILE("-noargs")<<"Execution of a simple test with an error when opening the data file"<<endl;
}

TEST_F(_03_Advanced, Integrated_ErrorData)  {   
    DEF_EXECUTION_ENVIRONMENT(ErrorData);
    USE_MEMORY_LEAKS(__VALGRIND__);
    SET_TIMEOUT_S(15);
    EXPECTED_OUTPUT(Data error in file ./data/ECommerce5.dataset);
    CALL_FROM_FILE("-noargs")<<"Execution of a simple test with an error when reading the data file"<<endl;
}

TEST_F(_03_Advanced, Integrated_ErrorSaving)  {   
    DEF_EXECUTION_ENVIRONMENT(ErrorSaving);
    USE_MEMORY_LEAKS(__VALGRIND__);
    SET_TIMEOUT_S(15);
    EXPECTED_OUTPUT(Error opening file ./datas/ECommerce5.dataset);
    CALL_FROM_FILE("-noargs")<<"Execution of a simple test with an error when saving the data file"<<endl;
}
#endif