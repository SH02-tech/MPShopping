...
TEST_F(_01_Basics, Pair_isEmpty) {
    Pair p;
    SKIP_ASSERT_TRUE_R(p.isEmpty())<< "Any newly created instance of Pair, without specifying any arguments, is considered to be emtpy" << endl;
    p.set("AA",0);
    SKIP_ASSERT_FALSE_R(p.isEmpty())<< "Any instance of Pair with valid key and pos, could not be considered emtpy" << endl;
}
