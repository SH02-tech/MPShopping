...
Matrix PageRank(const Matrix & M, int max_iter, double threshold, int n){
    ...
    string colLabels[nColumns];
    string rowLabels[1] = {"Brands"};
    M.getColLabels(colLabels);
    
    Matrix Pr(rowLabels, 1, colLabels, nColumns);
    
    for (int j=0; j<nColumns; j++) {
        int brand_col = M.colIndexOf(colLabels[j]);
        Pr(0, j) = M(M.rows()-1, brand_col);
    }
    
    bool stable = false;
    
    // Iterate until the max number of iteration (MAX_ITER) is reached or the 
    // convergency value is less than the threshold
    
    for (int i=0; i<max_iter && !stable; i++) {
        Matrix newPr = Pr * M;
        
        // We find the sum of all the differences, in absolute value. (1-norm)
        
        double sum = 0.0;
        
        for (int j=0; j<nColumns; j++)
            sum += abs(newPr(0,j) - Pr(0,j));
...
void createAdjacencyMatrix(const EventSet &evs, const Index & idx, Matrix & m_adj) {
    ...
    while (indPos<idx.size()) {
        string candidateSession = getEvent(evs, idx, indPos).getSession();
        
        if (candidateSession != session && !candidateSession.empty()) {
...
int main(int argc, char** argv) {
    ...
    // you should implement all the methods detailed in this file. Done!
...
void shopping5(int argc, char **argv ){
    ...
    Matrix pagerank;
    pagerank = computePageRank(m_ady, MAX_ITER, 0.00001);
